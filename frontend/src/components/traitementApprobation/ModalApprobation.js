...
export default function ModalApprobation(props) {
		...
		axios
			...
			.then(function (response) {
				if (response.status === 200 && response.data.success) {
					toast.success("Pré-VISA concéder avec success");
					i = 0;
					props.onHide();
				} else {
	...
	const validation = (event) => {
        ...
        if (!inputs.preVISA){
				...
				setErreurs((values) => ({ ...values, preVISA: true }));
				setMessages((values) => ({
					...values,
					preVISA: "Votre signature est vivement recommandée à cet endroit Chef.",
				}));
				isValidate = false;
        }
							...
							<Row>
								...
								<Col>
									<label className="form-check-label">
										...
										<span className="form-check-sign">
											<i>
												<BsNodePlusFill
													className={
														inputs.preVISA ? "text-info" : "text-white"
													}
												/>{" "}
											</i>
											Concéder un pré-VISA ?
									...
									<small className="text-danger d-block">
										{erreurs.preVISA ? messages.preVISA : null}
									</small>
								</Col>
							</Row>
						</Container>
